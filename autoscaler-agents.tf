        ...
        k3s_config = yamlencode(merge(
          {
            ...
            node-label    = concat(local.default_agent_labels, [for k, v in var.autoscaler_nodepools[count.index].labels : "${k}=${v}"], var.autoscaler_nodepools[count.index].swap_size != "" ? local.swap_node_label : [])
